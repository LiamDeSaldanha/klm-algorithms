...
const BASE_RANK_TRACE_URL = "/api/base-rank/trace";
...
const fetchBaseRankTrace = async (data: string[]) => {
  try {
    console.log("POST: BaseRank Trace Request: " + data.toString());

    const response = await axios.post(BASE_RANK_TRACE_URL, data);

    console.log("BaseRank Trace Response: " + response.data.toString());

    return response.data as ApiBaseRankTracer[];
  } catch (error) {
    throw getError(error);
  }
};

const fetchJustificationTrace = async (
  queryFormula: string,
  knowledgeBase: string[]
) => {
  try {
    console.log(
      "POST: Justification Trace Request: " +
        queryFormula +
        " " +
    ...
    const response = await axios.post(
      JUSTIFICATION_TRACE_URL(queryFormula),
      knowledgeBase
    );

    console.log("Justification Trace Response: " + response.data.toString());
